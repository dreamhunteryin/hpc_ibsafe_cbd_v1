...
class LoRAConfig:
    ...
    Args:
        ...
        target_module_match: How to match module names against target_modules
        ...
        apply_to_decoder_text_attention: Whether to apply LoRA to decoder text cross-attention
    ...
    def __init__(
        ...
        apply_to_decoder_text_attention: Optional[bool] = None,
        ...
        # - mask_mlp: MLP inside the segmentation head mask predictor
        if target_modules is None:
            target_modules = [
                ...
                "proj",  # Output projection in ViT blocks
                "fc1", "fc2",  # MLP layers when present
        ...
        if target_module_match not in {"exact", "substring"}:
            raise ValueError(
                "target_module_match must be either 'exact' or 'substring'"
            )
        self.target_modules = tuple(dict.fromkeys(target_modules))
        self.target_module_match = target_module_match
        ...
        self.apply_to_decoder_text_attention = (
            apply_to_text_encoder
            if apply_to_decoder_text_attention is None
            else apply_to_decoder_text_attention
        )
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "target_module_match": self.target_module_match,
            ...
            "apply_to_decoder_text_attention": self.apply_to_decoder_text_attention,
...
def apply_lora_to_model(model: nn.Module, config: LoRAConfig) -> nn.Module:
    ...
    def is_decoder_text_attention_module(module_name: str) -> bool:
        return module_name.endswith("ca_text") or ".ca_text." in module_name
    ...
    def should_apply_lora_to_component(module_name: str) -> bool:
        ...
        if (
            ...
            return False
        if is_decoder_text_attention_module(module_name):
            if not config.apply_to_detr_decoder:
                return False
            if not config.apply_to_decoder_text_attention:
                return False
        if (
            ...
            return False
        if (
            ("mask_decoder" in module_name or "segmentation_head" in module_name)
            and not config.apply_to_mask_decoder
        ):
            return False
        return True

    def matches_target_module(module_name: str) -> bool:
        module_basename = module_name.split(".")[-1]
        for target in config.target_modules:
            if target == "mask_mlp":
                if ".mask_predictor.mask_embed.layers." in module_name:
                    return True
            if (
                module_basename == target
                or module_name == target
                or module_name.endswith(f".{target}")
            ):
                return True
            if config.target_module_match == "substring" and target in module_basename:
                return True
        return False
    ...
    def should_apply_lora(module_name: str) -> bool:
        ...
        if not should_apply_lora_to_component(module_name):
            return False
        return matches_target_module(module_name)
...
def _get_lora_parameter_map(model: nn.Module) -> Dict[str, nn.Parameter]:
    parameter_map = {}
    for name, module in _iter_lora_modules(model):
        parameter_map[f"{name}.lora_A"] = module.lora_A
        parameter_map[f"{name}.lora_B"] = module.lora_B
    return parameter_map


def _get_lora_state_dict(model: nn.Module) -> Dict[str, torch.Tensor]:
    return {
        name: param.detach().cpu()
        for name, param in _get_lora_parameter_map(model).items()
    }


def save_lora_weights(model: nn.Module, save_path: str, config: Optional[LoRAConfig] = None):
    """
    ...
    Args:
        ...
        config: Optional LoRA config metadata to save alongside the weights
    """
    ...
    payload = {
        ...
        "metadata": {
            "format_version": 2,
            "expected_keys": sorted(lora_state_dict.keys()),
...
def _split_lora_payload(payload: Any) -> Tuple[Dict[str, torch.Tensor], Optional[Dict[str, Any]], bool]:
    if not isinstance(payload, dict):
        raise ValueError("Expected a LoRA checkpoint dictionary")
    if "state_dict" in payload:
        state_dict = payload["state_dict"]
        if not isinstance(state_dict, dict):
            raise ValueError("LoRA checkpoint 'state_dict' must be a dictionary")
        metadata = payload.get("metadata")
        if metadata is not None and not isinstance(metadata, dict):
            raise ValueError("LoRA checkpoint 'metadata' must be a dictionary when provided")
...
def load_lora_weights(
    model: nn.Module,
    load_path: str,
    expected_config: Optional[LoRAConfig] = None,
):
    """
    ...
    Args:
        ...
        expected_config: Optional LoRA config expected by the caller
    """
    payload = torch.load(load_path, map_location="cpu")
    lora_state_dict, metadata, is_legacy_format = _split_lora_payload(payload)
    current_params = _get_lora_parameter_map(model)
    current_keys = set(current_params.keys())
    saved_keys = set(lora_state_dict.keys())

    if metadata is not None:
        metadata_keys = metadata.get("expected_keys")
        if metadata_keys is not None and set(metadata_keys) != saved_keys:
            raise ValueError(
                "LoRA checkpoint metadata does not match the saved LoRA tensors. "
                f"metadata_keys={sorted(metadata_keys)} saved_keys={sorted(saved_keys)}"
            )
        saved_config = metadata.get("config")
        if expected_config is not None and saved_config is not None:
            expected_config_dict = expected_config.to_dict()
            if saved_config != expected_config_dict:
                raise ValueError(
                    "LoRA checkpoint config does not match the current LoRA config. "
                    f"saved={saved_config} current={expected_config_dict}"
                )

    missing_keys = sorted(current_keys - saved_keys)
    unexpected_keys = sorted(saved_keys - current_keys)
    if missing_keys or unexpected_keys:
        format_name = "legacy" if is_legacy_format else "metadata-aware"
        raise ValueError(
            f"LoRA checkpoint topology mismatch for {format_name} checkpoint. "
            f"missing_keys={missing_keys}, unexpected_keys={unexpected_keys}"
        )

    for name, parameter in current_params.items():
        tensor = lora_state_dict[name]
        if not isinstance(tensor, torch.Tensor):
            raise ValueError(f"LoRA tensor for {name} is not a torch.Tensor")
        if tensor.shape != parameter.shape:
            raise ValueError(
                f"Shape mismatch for {name}: checkpoint has {tuple(tensor.shape)}, "
